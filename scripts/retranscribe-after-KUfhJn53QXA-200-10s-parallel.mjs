...
async function main() {
  ...
  const [targets] = await bq.query({
    query: `
      ...
      LIMIT @limit
    `,
    params: { startAfter: START_AFTER, limit: LIMIT },
  });
  ...
  const outPath = path.resolve(process.cwd(), "scripts/retranscribe-after-KUfhJn53QXA-200-10s-parallel.results.json");
