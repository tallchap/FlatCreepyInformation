...
async function main() {
  ...
  const prevPath = path.resolve(process.cwd(), "scripts/retranscribe-first100-alpha.results.json");
  const prev = JSON.parse(fs.readFileSync(prevPath, "utf8"));
  const failedIds = prev?.summary?.failedIds || [];

  if (!failedIds.length) {
    console.log("No failed IDs found in prior results.");
    return;
  }
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    results.push({ videoId, status: passed ? "success" : "failed", attempts: attemptLogs });
  ...
  const failedIdsOut = results.filter((r) => r.status === "failed").map((r) => r.videoId);
  ...
  const summary = {
    ...
    successAfterRetry,
    failed,
    failedIds: failedIdsOut,
    attemptDelayMs: ATTEMPT_DELAY_MS,
  };

  const outPath = path.resolve(process.cwd(), "scripts/retranscribe-48-failures-delay.results.json");
