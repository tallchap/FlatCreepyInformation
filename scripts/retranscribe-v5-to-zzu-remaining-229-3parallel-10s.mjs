...
const START_AFTER = "v5pW_Mqt6dU";
const END_AT = "zzUdUQnZIIc";
const LIMIT = 229;
const CONCURRENCY = 3;
const MAX_ATTEMPTS = 2;
const ATTEMPT_DELAY_MS = 10000;
const VIDEO_DELAY_MS = 10000;
...
function shapeTranscript(videoId, transcriptData) {
  ...
  return { segments, segmentCount: segments.length, minStart, maxStart };
}
...
function passesCheckpoint({ segmentCount, maxStart }) {
  ...
  return { pass: segmentsPerMin >= MIN_SEGMENTS_PER_MIN, segmentsPerMin };
}
...
async function main() {
  ...
  const [targets] = await bq.query({
    query: `
      ...
      LIMIT @limit
    `,
    params: { startAfter: START_AFTER, endAt: END_AT, limit: LIMIT },
  });

  if (!targets.length) {
    console.log("No eligible videos found.");
    return;
  }

  console.log(`Selected ${targets.length} targets from ${targets[0].ID} to ${targets[targets.length - 1].ID}`);
    ...
    while (true) {
      ...
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
          ...
          attemptLogs.push({ attempt, ok: false, reason: "request_error", error: err?.message || String(err) });
          if (attempt < MAX_ATTEMPTS) await new Promise((r) => setTimeout(r, ATTEMPT_DELAY_MS));
        }
      }
  ...
  const summary = { batchSize: results.length, success, successAfterRetry, failed, failedIds };

  const outPath = path.resolve(process.cwd(), "scripts/retranscribe-v5-to-zzu-remaining-229-3parallel-10s.results.json");
