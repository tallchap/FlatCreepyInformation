...
## Twitter coverage — correction, 2026-09-09

Never present two first pages (roughly 40 posts) as a broad trend survey. The advanced-search API returns about 20 posts per page. Use twitter-survey.py with explicit date windows and a bounded page budget; it follows next_cursor while has_next_page is true, combines Top and Latest queries, and records stop reasons. The twitter-research-server helper also accepts max_pages; its default remains one for existing callers, so surveys must pass an explicit larger value.

Report requests, returned posts before deduplication, unique post IDs, query/window count, errors and how many queries stopped at the page cap versus exhausted results. A capped run is not exhaustive. Broaden topics and primary-source accounts, not just the raw count; inspect high-signal new leads and keep text posts distinct from verified video footage. Charge estimates must include all returned pages, including duplicate posts across queries, and retain earlier run costs.
