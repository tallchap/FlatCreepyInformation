...
async function main() {
  ...
  const [targets] = await bq.query({
    query: `
      ...
      WHERE s.rn = 1
        ...
        AND TRIM(s.ID) != ''
        AND d.id IS NULL
        AND s.ID NOT IN UNNEST(@skipIds)
      ORDER BY s.ID ASC
      LIMIT 100
    `,
    params: { skipIds: SKIP_FAILED_IDS },
  });
  ...
  const outPath = path.resolve(process.cwd(), "scripts/retranscribe-next100-10s-skipfailed.results.json");
  ...
  try {
    ...
    const merged = {
      ...
      source: "scripts/retranscribe-next100-10s-skipfailed.results.json",
      count: map.size,
